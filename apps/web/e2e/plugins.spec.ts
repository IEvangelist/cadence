...
test.describe('plugins / extensions', () => {
  ...
  test('enable the example plugin and run its command', async ({ page }) => {
    ...
    const toolTab = page.getByRole('tab', { name: 'Example plugin' })
    const toolPanel = page.getByRole('tabpanel', { name: 'Example plugin' })
    const toolTabId = await toolTab.getAttribute('id')
    expect(toolTabId).toBeTruthy()
    await expect(toolTab).toHaveAttribute('aria-controls', 'plugin-tool-panel')
    await expect(toolPanel).toHaveAttribute('id', 'plugin-tool-panel')
    await expect(toolPanel).toHaveAttribute('aria-labelledby', toolTabId!)
